...
info(){
	tput cnorm
	echo -e "
$(_magenta  "guilletester_ps Version 1.1 by gumoreno")


   * How to use it:
	
	$(_yellow_bold "./guilletester_ps.sh") $(_yellow "<how many ramdom numbers> <number of iterations>")	
			
		--> example:  ./guilletester_ps.sh 500 30  -> 30 iterations of 500 ramdom numbers

	$(_yellow_bold "./guilletester_ps.sh") $(_yellow "--strict")						
			
		--> special selection of push_swap destroyer 500 numbers combinations :)  
		            
			(try "$(_yellow_bold "./guilletester_ps.sh") $(_yellow "--pepestrict") "if you can't waste time)
...
print_to_order_log()
{
	if [ $op_sys = "Darwin" ]; then
		echo -e "Unsorted combination on "$1" ramdom numbers  ("$(date "+%c")")\nset ARG \""$ARG"\"; ./push_swap \$ARG | ./checker_Mac \$ARG\n" >> ./ps_log/unsorted_log_fish.txt
		echo -e "Unsorted combination on "$1" ramdom numbers  ("$(date "+%c")")\nARG=\""$ARG"\"; ./push_swap \$ARG | ./checker_Mac \$ARG\n" >> ./ps_log/unsorted_log_bash.txt
	elif [ $op_sys = "Linux" ]; then
		echo -e "Unsorted combination on "$1" ramdom numbers  ("$(date "+%c")")\nset ARG \""$ARG"\"; ./push_swap \$ARG | ./checker_linux \$ARG\n" >> ./ps_log/unsorted_log_fish.txt
...
print_results(){
	...
	if [[ $order_failed -ge 1 && $log = false ]]; then
		...
		echo -e "\n\r\033[1A"$(_red " ✘ ")" Order: "$order_failed" failed  \t\t---> Unsorted combinations saved in folder \"ps_log\"\033[1A"
...
do
	...
	if [ "$ORDER" = "KO" ]; then
		...
		error_clear
		echo -e "\n    Ups... Error, your push_swap not only prints operations :(\n "
		exit 41
	fi
